...
const NewHuntRecStep3 = ({ _id, navigation, preys, setPreys, imbuements, setImbuements, charms, setCharms }) => {
  ...
  const attackImbuements = Imbuements.filter(imbuement => imbuement.type === 'Attack')
  const protectiveImbuements = Imbuements.filter(imbuement => imbuement.type === 'Protective')
  const supportImbuements = Imbuements.filter(imbuement => imbuement.type === 'Support')
  const skillsImbuements = Imbuements.filter(imbuement => imbuement.type === 'Skills')

  const imbuementsForm = <div className="form imbuements">
      <div className="form-input-row">
        <div className="flex-row">
        <select name="name" onChange={(e) => setSelectedImbuement({...selectedImbuement, [e.target.name]: e.target.value})}>
            <optgroup label="Attack">
              {
                attackImbuements.map((imbuement, index) => {
                  return <option key={index} value={imbuement.name}>{imbuement.name}</option>
                })
              }
            </optgroup>
          ...
          <input type="number" min="1" name="ammount" onChange={(e) => setSelectedImbuement({...selectedImbuement, [e.target.name]: e.target.value})} />
          <i className="fas fa-plus-circle" onClick={() => setImbuements(imbuements.concat(selectedImbuement))} />
        </div>
      </div>
      <div className="form-input-row">
				{
					imbuements && (
						imbuements.map((imbuement, index) => {
							return (
								<p key={index} onClick={(e) => removeIndexImbuement(e, index)}>
  ...
  const charmsForm = <div className="form charms">
    <div className="form-input-row">
        <div className="flex-row">
        <select name="name" onChange={(e) => setSelectedCharm(e.target.value)}>
              {
                Charms.map((charm, index) => {
                  return <option key={index} value={charm.name}>{charm.name}</option>
                })
              }
          </select>
          <i className="fas fa-plus-circle" onClick={() => setCharms(charms.concat(selectedCharm))} />
        </div>
      </div>
    <div className="form-input-row">
				{
					charms && (
						charms.map((charm, index) => {
							return (
								<p key={index} onClick={(e) => removeIndexCharm(e, index)}>
									{charm}
								</p>
							)
						})
  ...
  const content = (
         <Fragment>
          <div id="formContainer">
            ...
            <FormBox title="Imbuements" imgSrc="../images/Fire_Sword.gif" form={imbuementsForm} margin="10px" />
            <FormBox title="Charms" imgSrc="../images/Charm_Expansion.png" form={charmsForm} margin="10px" />
          </div>
